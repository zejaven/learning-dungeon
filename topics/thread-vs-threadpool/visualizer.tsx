...
const LABELS = {
  runHint: {
    en: 'Run the code to visualize Thread and ThreadPool behavior.',
    ru: 'Запустите код, чтобы увидеть поведение Thread и ThreadPool.',
  },
  standaloneThreads: { en: 'new Thread per task', ru: 'новый Thread на задачу' },
...
const ACTION_LABELS: Record<string, Localized> = {
  CREATE_NEW_THREAD: { en: 'created new Thread', ru: 'создал новый Thread' },
  START_NEW_THREAD: { en: 'started Thread', ru: 'запустил Thread' },
  TERMINATE_NEW_THREAD: { en: 'terminated Thread', ru: 'завершил Thread' },
...
interface ThreadPoolState {
  name: string;
  standaloneThreads: StandaloneThread[];
  pools: PoolSnapshot[];
  history: HistoryItem[];
}

export default function ThreadVsThreadPoolVisualizer({ event }: VisualizerProps) {
  ...
  const standaloneNodes: LinkedNode[] = state.standaloneThreads.map((thread) => {
    const status = STATE_LABELS[thread.state] ?? { en: thread.state, ru: thread.state };
    return {
      id: thread.name,
      title: thread.name,
      subtitle: `${tl(status, lang)} - ${tl(LABELS.task, lang)} ${thread.task}`,
      highlighted: highlight.has(`thread:${thread.name}`) || highlight.has(`task:${thread.task}`),
    };
  });
  ...
  const topCells: ArrayCell[] = [
    {
      key: 'standalone',
      label: tl(LABELS.standaloneThreads, lang),
      content: <LinkedNodes nodes={standaloneNodes} />,
    },
    {
      key: 'pools',

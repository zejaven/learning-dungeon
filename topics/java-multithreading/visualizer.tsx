...
const LABELS = {
  runHint: {
    en: 'Run the code to visualize Java thread lifecycle events.',
    ru: 'Запустите код, чтобы увидеть события lifecycle Java thread.',
  },
  currentThread: { en: 'current thread', ru: 'текущий thread' },
  tasks: { en: 'Runnable tasks', ru: 'задачи Runnable' },
  workers: { en: 'Java Thread objects', ru: 'объекты Java Thread' },
  ...
  noThread: { en: 'not attached', ru: 'не привязан' },
...
export default function JavaMultithreadingVisualizer({ event }: VisualizerProps) {
  ...
  const workerNodes: LinkedNode[] = state.threads.map((thread) => {
  ...
  const cells: ArrayCell[] = [
    ...
    {
      key: 'tasks',
      label: tl(LABELS.tasks, lang),
      content: <LinkedNodes nodes={taskNodes} />,
    },
    {
      key: 'workers',
      label: tl(LABELS.workers, lang),
      content: <LinkedNodes nodes={workerNodes} />,
    },
  ];
